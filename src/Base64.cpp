...
extern "C" K base64decode(K toDecode) {
    if(toDecode->t != KC)
        return krr((char*) "[lib-kdbbase64] Invalid type for Base64 decode. Must be string");
...
extern "C" K base64encode(K toEncode) {
	if(toEncode->t != KC)
		return krr((char*) "[lib-kdbbase64] Invalid type for Base64 encode. Must be string");
